...
using EB.Application.Features.Accounts.Queries;
using EB.Application.Features.Members.Commands;
...
public class IdentityService : IIdentityService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var userClaims = claims.Select(x => new ClaimDto ("",x.Type, x.Value)).ToList();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        int counter = 1;
        var userClaims = claims.Select(x => new ClaimDto(counter++.ToString(),x.Type,x.Value)).ToList();
